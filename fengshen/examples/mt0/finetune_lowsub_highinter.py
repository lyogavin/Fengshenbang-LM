...
from data.task_dataloader.LowsubHighinterMTDataset import LowsubHighinterDataModel
...
class GPT2FinetuneMedicalQA(pl.LightningModule):
    ...
    def validation_epoch_end(self, training_step_outputs):
        ...
        for bi, item in enumerate(additional_cases):
            ...
            #prompted_content = f"根据以下内容给出小红书笔记标题：{item}"
            prompted_content = f"根据以下内容给出小红书笔记标题：{item}"
            prompt_inputs_dict = self.tokenizer.batch_encode_plus([prompted_content],
                                                            max_length=800, padding=False,
...
def main():
    ...
    if not args.do_eval_only:
        ...
        logger = loggers.TensorBoardLogger(save_dir=os.path.join(
            args.default_root_dir, 'log/'), name='mt0')
        ...
        model.model.save_pretrained(os.path.join(args.default_root_dir, f"mt0_lowsub_highinter_{args.run_ts}"))
           # '/cognitive_comp/wuziwei/pretrained_model_hf')
        print(f'model saved: {os.path.join(args.default_root_dir, f"mt0_lowsub_highinter_{args.run_ts}")}')
    else:
        print('save to hf.....')
        trainer = Trainer.from_argparse_args(args)
