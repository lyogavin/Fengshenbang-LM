...
class GPT2QADataset(Dataset):
    ...
    def __init__(self, data_path, name, args):
        ...
        self.max_label_seq_length = args.max_label_seq_length
    ...
    def encode(self, item):
        ...
        prompted_content = f"根据以下内容给出\"{item['title_template_name']}\"类型的标题：{item['content']}"
        ...
        labels_dict = self.tokenizer.encode_plus(item['title'],
                                                 max_length=self.max_label_seq_length, padding='max_length',
                                                 truncation=True, return_tensors='pt')
        #prompt_inputs_dict = self.tokenizer.encode_plus(item['prompt'],
        #                                         max_length=self.max_seq_length, padding=False,
        #                                         truncation=True, return_tensors='pt')
        target = labels_dict['input_ids']
...
class TitleTemplateDataModel(pl.LightningDataModule):
    @staticmethod
    def add_data_specific_args(parent_args):
        ...
        parser.add_argument('--max_label_seq_length', default=80, type=int)
        return parent_args
