...
class GPT2QADataset(Dataset):
    ...
    def encode(self, item):
        ...
        def get_promote_title_type(row):
            ...
            if row['data_type'] in ['redbook_tags_title', 'redbook_tags_title_top2_sim']:
                return f"小红书"
            elif row['data_type'] in ['redbook_content_title']:
                return f"小红书"
            elif row['data_type'] in ['zhihu_article']:
                return f"微信公众号"
            elif row['data_type'] in ['zhihu_question']:
                return f"知乎标题"
            # by source category
            elif row['source_category'] in ['zhihu_search', 'zhihu_fin_topic_download']:
                return f"知乎标题"
            elif row['source_category'] in ['newrank_healthcare', 'newred_0905','redbook_0707']:
                return f"小红书"
            elif row['source_category'] in ['newred_0905_ocr']:
                return f"小红书首图"
            elif row['source_category'] in ['baidubaijia', 'newrank_hot_wx_account_0929', 'snowball',
                                            'sougou_wx', 'sougou_wx_search_expansion']:
                return f"微信公众号"
            elif row['source_category'] in ['xunyiwenyao', 'zhihu_search_exp_article',
                                            'zhihu_fin_topic_download','zhihu_search_exp_question', 'zhihu_search_exp_article']:
                assert False
            elif row['source_category'] in ['taobao_jd']:
                return f"电商"
            else:
...
if __name__ == '__main__':
    ...
    testml = GPT2QADataset("/home/ubuntu/cloudfs/ghost_data/newred_redbook_link_download/api_0305_download/all_merged_til0305_cleanedup_train_1679092112.csv.tgz", 'medical_qa', args=args)
    ...
    testml = GPT2QADataset(
        "/home/ubuntu/cloudfs/ghost_data/newred_redbook_link_download/api_0305_download/all_merged_til0305_cleanedup_val_1679092112.csv.tgz",
